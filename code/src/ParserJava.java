...
public class ParserJava implements ParserIf
{
	...
	while (interfaceMatcher.find())
	{
	    ...
	    interfaceConnection.add(interfaceMatcher.group());	//speichere die Zeile um später die Interfaces auszulesen die in Verbindung zu dem Inerface stehen
	...
	for (int i = 0; i < interfaceConnection.size(); i++)		//suche nach extends, da Interfaces von einem oder mehreren anderen Interfaces erben können
	{
	    ...
	    {
		exInterfaceConnection(i);			// Funktion die alle Verbindungen des Interfaces i zu den anderen Interfaces rausliest und speichert
	    }
	}
...
//    }
	...
	ClassConnection cc;
	// Pattern um den Namen auszulesen ersetllen
	...
	//hinzufügen der Verbindung ins Array
	...
	if (mImp1.find())
	{
		...
		System.out.println(interface1 + " ist Interface von " + className.get(i));
		//hinzufügen der Verbindung zum Array
		cc = new ClassConnection((i + countIN), bothNumber(interface1), extensionType);
		classConnectionArray.add(cc);
		System.out.println("G " + classConnectionArray.size());
	    }
	...
	ClassConnection cc;
	//erstellen des Patterns um bei Interfaces auszulesen ob es von eineme oder mehreren Interfaces "erbt"
	...
	if (mEx1.find())
	{
	    ...
	    // bei folgendem "," ist von folgenden Interfacenamen auszugehen, die man noch auslesen und hinzufügen muss
		...
		System.out.println(interface1 + " ist Interface von " + bothName.get(i));
		//Neue Verbindung wird hinzugefügt
		cc = new ClassConnection((i), bothNumber(interface1), extensionType);
		classConnectionArray.add(cc);
	    }
	    System.out.println("test");
